...
m=[0.18,0.36,0.54,0.63,0.9,0.99];
ObjV=zeros(8,1);
A=zeros(6,3);
...
Beta = trace(3,2);
for i=2:6
    ment(i,:)=1/2*(Alpha*ment((i-1),:)+B*Beta);
end
for i=1:6
    Theta(i)=((ment(i,1)*ment(i,1)+ment(i,2)*ment(i,2)+ment(i,3)*ment(i,3))^(1/2))/3^(1/2);
end
end
